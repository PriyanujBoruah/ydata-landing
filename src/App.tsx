...
import SolutionBriefModal from "./components/SolutionBriefModal";
...
export default function App() {
  ...
  const [contactForm, setContactForm] = useState({ name: "", email: "", corp: "", msg: "" });
  ...
  const handleContactSubmit = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      ...
      setContactForm({ name: "", email: "", corp: "", msg: "" });
    }, 4000);
  };
    ...
    <div className="bg-[#FCFAF8] text-slate-900 font-sans min-h-screen relative overflow-x-hidden selection:bg-orange-100 selection:text-orange-900" id="applet-viewport">
      ...
      >
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          ...
          {/* Logo Brand Block co-branded with IIT Madras */}
          <div className="flex items-center gap-3">
            ...
            {/* Tiny co-branding subtext */}
            <div className="hidden lg:flex items-center gap-1.5 pl-2 border-l border-slate-200">
              <GraduationCap className="w-4 h-4 text-orange-500" />
              <span className="text-[10px] text-slate-500 font-semibold tracking-tight">Academic origin: <strong>IIT Madras</strong></span>
            </div>
          </div>
          ...
          <nav className="hidden md:flex items-center gap-6 text-xs font-semibold text-slate-600">
            ...
            <button onClick={() => scrollToSection("contact-section")} className="hover:text-orange-500 transition cursor-pointer">Deploy Workspace</button>
          </nav>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-center relative z-10">
          ...
          <div className="lg:col-span-7 bg-slate-800/55 rounded-2xl p-5 md:p-6 border border-slate-700/60 shadow-lg space-y-4">
            ...
            <div className="bg-slate-950 p-4 rounded-xl text-center space-y-2">
              ...
              <button
                onClick={() => setIsBriefOpen(true)}
                className="bg-orange-500 hover:bg-orange-600 text-white font-bold text-xs px-4 py-2 rounded-lg transition"
              >
      ...
      <section className="py-16 px-4 md:px-8 max-w-3xl mx-auto" id="contact-section">
        
        <div className="bg-gradient-to-br from-white to-slate-50 rounded-3xl border border-orange-100 shadow-xl p-6 md:p-8 space-y-6">
          <div className="text-center space-y-2">
            <h3 className="text-2xl font-black text-slate-900">Request Sandbox Access</h3>
            <p className="text-xs text-slate-500 leading-relaxed max-w-md mx-auto">
              Get an offline demonstration running directly inside your firm's local browser boundaries or inquire about specialized private instance deployments.
            </p>
          </div>

          {demoRequested ? (
            <div className="bg-emerald-50 border border-emerald-100 rounded-2xl p-6 text-center space-y-3 animate-pulse">
              <CheckCircle className="w-10 h-10 text-emerald-500 mx-auto" />
              <h4 className="font-extrabold text-sm text-emerald-950">Indian Workspace Allocation Triggered</h4>
              <p className="text-xs text-emerald-700 max-w-xs mx-auto">
                Thank you! The founding team will reach out to you within 2 business hours. Check your inbox for compliance credentialing paperwork.
              </p>
            </div>
          ) : (
            <form onSubmit={handleContactSubmit} className="space-y-4">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="space-y-1">
                  ...
                  <input
                    ...
                    className="w-full bg-white text-xs border border-slate-200 px-3 py-2.5 rounded-xl focus:border-orange-500 outline-none transition"
                  />
                </div>
                <div className="space-y-1">
              ...
              <div className="space-y-1">
                <label className="text-[10px] font-bold text-slate-500 uppercase">Organization / Corporation Name</label>
                <input
                  type="text"
                  required
                  value={contactForm.corp}
                  onChange={(e) => setContactForm({ ...contactForm, corp: e.target.value })}
                  placeholder="e.g. Bangalore Fintech Ventures"
                  className="w-full bg-white text-xs border border-slate-200 px-3 py-2.5 rounded-xl focus:border-orange-500 outline-none transition"
                />
              </div>

              <div className="space-y-1">
                <label className="text-[10px] font-bold text-slate-500 uppercase">Operational Bottleneck Profile</label>
                <textarea
                  required
                  rows={3}
                  value={contactForm.msg}
                  onChange={(e) => setContactForm({ ...contactForm, msg: e.target.value })}
                  placeholder="Describe your current spreadsheet, GST reconciliation, or compliance bottlenecks..."
                  className="w-full bg-white text-xs border border-slate-200 p-3 rounded-xl focus:border-orange-500 outline-none transition resize-none"
                />
              </div>

              <button
                type="submit"
                className="w-full bg-orange-500 hover:bg-orange-600 text-white font-extrabold text-xs py-3 rounded-xl transition shadow flex items-center justify-center gap-1.5 cursor-pointer"
              >
                <Send className="w-3.5 h-3.5" /> Allocate Local Sandbox Node
              </button>
            </form>
          )}

          <div className="pt-3 border-t text-center text-[10px] text-slate-400 space-y-1">
            <p>Direct Email Contact: <a href="mailto:22f3000795@ds.study.iitm.ac.in" className="text-orange-500 font-bold hover:underline">22f3000795@ds.study.iitm.ac.in</a></p>
            <p>Academic Research Archive Number: doi.org/10.7910/DVN/FZMD31</p>
          </div>
      ...
      <footer className="bg-slate-900 text-slate-400 py-12 px-4 md:px-8 text-xs border-t border-slate-800">
        <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-12 gap-8 items-start mb-8">
          ...
          <div className="md:col-span-5 space-y-3.5">
            ...
            <div className="flex items-center gap-2 text-[10px] text-slate-300">
              ...
              <span>Developed as an indigenous initiative inside Chennai computing groups.</span>
            </div>
          </div>
          ...
          <div className="md:col-span-3 space-y-3">
            ...
            <ul className="space-y-2 text-[11px]">
              <li>
                <a href="https://doi.org/10.7910/DVN/FZMD31" target="_blank" rel="noopener noreferrer" className="hover:text-white flex items-center gap-1 transition">
                  Harvard Dataverse Thesis <ExternalLink className="w-3 h-3 text-orange-500" />
                </a>
              </li>
              <li>
                <span className="block text-slate-500">Research DOI: doi.org/10.7910/DVN/FZMD31</span>
              </li>
              <li>
                <span className="block text-slate-300">IIT Madras Computing Affiliation</span>
              </li>
            </ul>
          </div>
          ...
          <div className="md:col-span-4 space-y-3">
            ...
            <ul className="space-y-2 text-[11px]">
              ...
              <li>
                <span>Contact Email: <a href="mailto:22f3000795@ds.study.iitm.ac.in" className="text-slate-200 underline hover:text-orange-400">22f3000795@ds.study.iitm.ac.in</a></span>
              </li>
            </ul>
          </div>
        ...
        <div className="max-w-7xl mx-auto pt-6 border-t border-slate-800 text-center md:text-left flex flex-col md:flex-row justify-between items-center gap-4 text-[10px] text-slate-500">
          ...
          <div className="flex gap-4">
            <button onClick={() => setIsBriefOpen(true)} className="hover:text-white cursor-pointer select-none">Review Solution Brief PDF</button>
      ...
      {/* Main Solution Brief Modal View */}
      <SolutionBriefModal isOpen={isBriefOpen} onClose={() => setIsBriefOpen(false)} />
